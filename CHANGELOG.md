...
## 2024.10.1-geoplanetary.10

### General
- Enhance: ハンドルネームに含まれる情報量を計算可能に
	* 有効にすると、ユーザーモデレーションページに計算された情報量が表示されるようになります。
	* 計算のためには、計算用のテーブルを作成し、`admin/update-meta`で登録する必要があります。
- Enhance: コンディショナルロールの条件をさらに拡充
	* ハンドルネームの情報量、フィールド(追加情報)を条件に組み込めるようになります。
- Enhance: 予約ユーザー名に正規表現を使用可能に
- Enhance: EmailのBAN機能を強化

## 2024.10.1-geoplanetary.9

### General
- Enhance: コンディショナルロールの条件をさらに拡充
	* ユーザータグ、メールアドレス、二要素認証、ログイン日数、bio画像の設定とblurhashを条件に組み込めるようになります。
	* パフォーマンス上の理由から、一部の条件はローカルユーザーに対してのみ機能します。
- Enhance: ポリシーによる制御の範囲を拡大
	* これにより、ドライブの変更、クリップ機能、リスト機能、ファイルの添付と連合をロールで制御可能になります。
...
### Client
- Enhance: 迷惑になる可能性のある投稿の警告方法をダイアログからポップ表示に
- Enhance: ブロックしている/凍結されているユーザーのフォローボタンを一部で非表示に
- Enhance: 投稿フォームのプレビューデザインを選択可能に
...
### Note
- `2024.2.0-geoplantery.6`で試験的に追加された「AP Inboxでモデレーションにより作成失敗した投稿をキューに戻さないように」はアップストリームにて実装された同様の機能を取り込むため、削除されました
- `2024.2.0-geoplantery.5`で追加された「コンディショナルロールの条件の拡充」の一部はアップストリームにて同様の機能を実装したため、統合されました
  - コンディショナルロールの条件で「マニュアルロールにアサイン済み」を使用している場合は再設定が必要です
- このアップデートで副作用のあるオブジェクト名変更がされています
  - 投稿禁止パターンのルールに「ロールにアサイン済み」を使用している場合は再設定が必要です
...
- Fix: 禁止ノートパターンで、ハッシュタグのマッチができないのを修正
- Enhance: 禁止ノートパターンで、複数のノートやタグのマッチをまとめて記述できるように
...
## Note
- このリリースから、アップストリームにないデータベースの変更が行われているため、アップストリームとのデータベースの互換性が失われる可能性があります。

## General
- Feat: 禁止ノートパターン機能を実装
  * 2024.2.0-beta.12 で追加された禁止ワード機能の拡張版です
  * 禁止ワードではできない柔軟な投稿制限が可能になりますが、式が複雑になるほど負荷が増えるため、禁止ワード機能でカバーできない状況での使用のみに留めることをお勧めします。
- Enhance: ロールポリシーでDMを制限可能に
- Enhance: 返信/DMの制限があっても、自分自身には返信/DMできるように
- Enhance: コンディショナルロールの条件の拡充
  * 名前がマッチしているか、指定されたマニュアルロールにアサインされているかを条件に含めることができるようになりました

## 2024.2.0-geoplantery.4

### General
- Enhance: コンディショナルロールの条件の拡充
  * 連合しているサーバーのユーザーであるかどうかを条件に含めることができるようになりました
- Enhance: ロールポリシーでRenote/返信を制限可能に
  * Renoteには引用が、返信にはメンションが含まれます
...
- Enhance: 投稿できる最大の文字数を65536文字に引き上げ
	- Note: メインラインのMisskeyでは、投稿に8192文字のハードリミットがあるため、連合先では8192文字以降は切り捨てられます
- Enhance: ロールで投稿の可否と投稿できる最大の文字数を設定可能に
- 新しい実績を追加

### Client
- Enhance: 長いノートをオーバーフローさせて表示する機能を追加
- Enhance: 投稿フォームでノートの文字数表示を3種類から選択可能に
...
## 13.12.2-geoplanetary-1

### General
- 投稿したコンテンツのAIによる学習を軽減するオプションのデフォルト値を変更
